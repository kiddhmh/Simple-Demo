...
static NSString *CacheURLHeader = @"CacheURLProtocolCache";

static NSSet *CachingSupportSchemes;

static NSString * const URLProtocolHandleKey = @"URLProtocolHandleKey";

static NSString * const CacheURLStringKey = @"CacheURLStringKey"; // 本地保存缓存urlKey的数组key


@interface MHURLProtocol ()<NSURLConnectionDelegate>

@property (nonatomic, strong) NSURLConnection *connection;
@property (nonatomic, strong) NSMutableData *data;
@property (nonatomic, strong) NSURLResponse *response;

@property (nonatomic, strong) MHCacheModel *cacheModel;

@end

@implementation MHURLProtocol

- (MHCacheModel *)cacheModel{
    if (!_cacheModel) {
        _cacheModel = [[MHCacheModel alloc] init];
    }
    return _cacheModel;
}

+ (void)initialize{
    if (self == [MHURLProtocol class]){
        CachingSupportSchemes = [NSSet setWithObjects:@"http", @"https", nil];
    }
}

+ (BOOL)canInitWithRequest:(NSURLRequest *)request{
    if ([CachingSupportSchemes containsObject:[[request URL] scheme]] &&
        ([request valueForHTTPHeaderField:CacheURLHeader] == nil)){
...
- (void)startLoading{
    //打标签，防止无限循环
    [NSURLProtocol setProperty:@YES forKey:URLProtocolHandleKey inRequest:[[self request] mutableCopy]];
    
    // 加载本地
    MHCacheModel *cacheModel = (MHCacheModel *)[[MHCache defaultcache] objectForKey:[[[self.request URL] absoluteString] cd_md5HexDigest]];
...
- (void)cacheDataWithResponse:(NSURLResponse *)response  redirectRequest:(NSMutableURLRequest *)redirectableRequest{
    ...
    [[MHCache defaultcache] setObject:self.cacheModel forKey:cacheStringkey withBlock:^{
...
- (void)loadCacheData:(MHCacheModel *)cacheModel{
    if (cacheModel) {
